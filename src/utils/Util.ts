...
export default class Util {
  ...
  static sortAlphabet = (a: { DisplayName: any }, b: { DisplayName: any }) => {
    if (this.replaceVietnamese(a.DisplayName) > this.replaceVietnamese(b.DisplayName)) {
      return 1
    }
    if (this.replaceVietnamese(a.DisplayName) < this.replaceVietnamese(b.DisplayName)) {
      return -1
  ...
  static findItemByIdInOptions = (code: string | any, options: any, propertyGet: string): any => {
    try {
      ...
      const item: any = options?.find((el: any) => el?.[propertyGet] == code)
      if (!item) return null
      return item
    } catch (error: unknown) {
  ...
  static setDfValueWhenNull = (value: any, defaultValue: any = "") => {
    if (this.isEmpty(value) || value == 0 || value == "0" || isNaN(value)) {
      return defaultValue
    }
    return value
  }
  ...
  static findItemIndexByIdInOptions = (code: string | any, options: any, propertyGet: string): any => {
    try {
      ...
      const item: any = options?.findIndex((el: any) => el?.[propertyGet] == code)
